/**
 * Shared Postgres TLS options for Sequelize (dialectOptions) and node-pg (Pool.ssl).
 *
 * Env:
 *   POSTGRES_SSL_REJECT_UNAUTHORIZED=true  — verify server cert (recommended prod + RDS)
 *   POSTGRES_SSL_REJECT_UNAUTHORIZED=false — encrypt only (legacy / some managed hosts)
 *   POSTGRES_SSL_CA_PATH                   — PEM file path (overrides bundled AWS RDS CA)
 *   POSTGRES_SSL_CA                        — inline PEM (multiline env)
 *   POSTGRES_SSL_MODE=disable              — no TLS (local dev only)
 */
const fs = require("fs");
...
/**
 * @returns {{ dialectOptions: object, poolSsl: object|undefined, mode: string, source: string|null }}
 */
function buildPostgresSsl(env = process.env) {
  ...
  if (rejectFlag === "true") {
    if (!caPem) {
      throw new Error(
        "POSTGRES_SSL_REJECT_UNAUTHORIZED=true requires a CA bundle. " +
          "Set POSTGRES_SSL_CA_PATH or use the bundled services/utils/global-bundle.pem via monorepo checkout."
      );
    }
    ...
    // Default prod path: verify when AWS/RDS bundle is available.
    rejectUnauthorized = true;
  }

  const ssl = caPem && rejectUnauthorized
...
function logPostgresSsl(env = process.env) {
  ...
  console.warn(
    "[postgres ssl] TLS encryption without certificate verification (rejectUnauthorized: false). " +
      "Set POSTGRES_SSL_REJECT_UNAUTHORIZED=true and POSTGRES_SSL_CA_PATH for strict mode."
  );
}
